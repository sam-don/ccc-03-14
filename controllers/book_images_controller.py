...
def book_image_create(book_id, user=None):
    book = Book.query.filter_by(id=book_id, user_id=user.id).id.first()
    ...
    if Path(image.filename).suffix not in ['.png', '.jpg', '.jpeg', '.gif']:
...
def book_image_delete(book_id, id, user=None):
    ...
    if not book:
        return abort(400, description="Invalid book")
